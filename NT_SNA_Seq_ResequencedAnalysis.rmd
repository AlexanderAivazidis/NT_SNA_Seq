---
title: "Single Nucleus Sequencing Trial RESEQUENCED DATA - Quality Control and Analysis"
output:
...
mkdir data/snQCandAnalysis/OCT1_10x_resequenced/  
mkdir data/snQCandAnalysis/FFT4G_10x_resequenced/  
mkdir data/snQCandAnalysis/FFT4G_NEB_resequenced/  
...
rsync -avzhr aa16@farm3-login:/nfs/team283/sequencing/snSeq/cellranger302_count_30636_5705STDY7945423_mm10-3_0_0_premrna/filtered_feature_bc_matrix /home/jovyan/data/snQCandAnalysis/FFT4G_10x_resequenced/  
gunzip /home/jovyan/data/snQCandAnalysis/FFT4G_10x_resequenced/filtered_feature_bc_matrix/features.tsv.gz

rsync -avzhr aa16@farm3-login:/nfs/team283/sequencing/snSeq/cellranger302_count_30636_5705STDY7945424_mm10-3_0_0_premrna/filtered_feature_bc_matrix /home/jovyan/data/snQCandAnalysis/OCT1_10x_resequenced/
gunzip /home/jovyan/data/snQCandAnalysis/OCT1_10x_resequenced/filtered_feature_bc_matrix/features.tsv.gz  
...
# 10x data:
data_10xF = readMM('/home/jovyan/data/snQCandAnalysis/FFT4G_10x_resequenced/filtered_feature_bc_matrix/matrix.mtx.gz')
rowdata_10xF = read.delim('/home/jovyan/data/snQCandAnalysis/FFT4G_10x_resequenced/filtered_feature_bc_matrix/features.tsv', header = FALSE)
coldata_10xF = read.delim('/home/jovyan/data/snQCandAnalysis/FFT4G_10x_resequenced/filtered_feature_bc_matrix/barcodes.tsv.gz', header = FALSE)
data_10xF = as.matrix(data_10xF)
rownames(data_10xF) = rowdata_10xF[,2]
colnames(data_10xF) = coldata_10xF[,1]

data_10xO = readMM('/home/jovyan/data/snQCandAnalysis/OCT1_10x_resequenced/filtered_feature_bc_matrix/matrix.mtx.gz')
rowdata_10xO = read.delim('/home/jovyan/data/snQCandAnalysis/OCT1_10x_resequenced/filtered_feature_bc_matrix/features.tsv', header = FALSE)
coldata_10xO = read.delim('/home/jovyan/data/snQCandAnalysis/OCT1_10x_resequenced/filtered_feature_bc_matrix/barcodes.tsv.gz', header = FALSE)
...
Another QC metric is the expression of apoptosis marker, which I checked for next and turned out to be 0 in most cells:

```{r, include = TRUE, message = FALSE, warning = FALSE, cache = TRUE}

apoProp0 = colSums(data_NEB[c('ENSMUSG00000024401', 'ENSMUSG00000025408', 'ENSMUSG00000027712'),])/colSums(data_NEB)
apoProp1 = colSums(data_10xF[c('Tnf', 'Ddit3', 'Anxa5'),])/colSums(data_10xF)
apoProp2 = colSums(data_10xO[c('Tnf', 'Ddit3', 'Anxa5'),])/colSums(data_10xO)

apoDataFrame = data.frame(apoProp = c(apoProp0, apoProp1, apoProp2),
                         Run = c(rep('FFT4G_NEB', length(apoProp0)), rep('FFT4G_10x', length(apoProp1)), rep('OCT1_10x', length(apoProp2))))

p0 <- ggplot(apoDataFrame, aes(x=Run, y=apoProp, fill = Run)) + 
  scale_y_log10() +
  geom_violin() + 
  geom_jitter(shape=16, size = 0.45, width = 0.45, height = 0.05) +
  stat_summary(fun.y=mean, geom="point", size=2, color="red") +
  ylab('Apoptosis Gene RNA Counts Proportion') +
  theme(axis.text=element_text(size=14),
        axis.title=element_text(size=14,face="bold"),
        legend.text=element_text(size=14))
p0
pdf(file = 'figures/apoptosisGeneCounts.pdf', width = 10, height = 10)
p0
dev.off()
...
knitr::include_graphics("scrublet/FFT4G_10x_resequenced_doubletScore_histogram.pdf")
knitr::include_graphics("scrublet/OCT1_10x_resequenced_doubletScore_histogram.pdf")
...
doublets_10xF = read.table('scrublet/FFT4G_10x_resequenced_predicted_doublets.txt')
doublets_10xO = read.table('scrublet/OCT1_10x_resequenced_predicted_doublets.txt')
doubletsScore_10xF = read.table('scrublet/FFT4G_10x_resequenced_doublets_scores.txt')
doubletsScore_10xO = read.table('scrublet/OCT1_10x_resequenced_doublets_scores.txt')
...
doubletsScore_10xF = doubletsScore_10xF[keep10xF]
doubletsScore_10xO = doubletsScore_10xO[keep10xO]
doubletsScore_NEB = doubletsScore_NEB[keepNEB]

apoProp1 = apoProp1[keep10xF]
apoProp2 = apoProp2[keep10xO]
apoProp0 = apoProp0[keepNEB]
...
visualCortex.list[[2]]$doubletScore = doubletsScore_10xF
visualCortex.list[[3]]$doubletScore = doubletsScore_10xO
visualCortex.list[[4]]$doubletScore = doubletsScore_NEB

visualCortex.list[[2]]$apoProp = apoProp1
visualCortex.list[[3]]$apoProp = apoProp2
visualCortex.list[[4]]$apoProp = apoProp0
...
p19 = FeaturePlot(visualCortex.10xintegrated, feature = "doubletScore", reduction = "umap", cols = c('yellow', 'red'))
p20 = FeaturePlot(visualCortex.10xintegrated, feature = "apoProp", reduction = "umap", cols = c('yellow', 'red'))
p15
p16
p17
p18
...
p18
dev.off()
pdf(file = 'figures/UMAP_10x_doubletScore.pdf', width = 10, height = 5)
p19
dev.off()
pdf(file = 'figures/UMAP_10x_apoProp.pdf', width = 10, height = 5)
p20
dev.off()
```

The number of counts, classification scores and mitochondrial counts fraction are not strongly out of the expected range. So it may simply be that this is a further subcategory of glutamatergic_L5_PT neurons that is only present in the OCT samples. Isolating the cells in this cluster and looking at DE genes compared to the other cells of the same type is the last step, where we find 274 differentially expressend genes, none of which show any suspicious enrichment for certain GO processes (e.g. apoptosis):

```{r, include = TRUE, message = FALSE, warning = FALSE, cache = FALSE}

oddCluster = p15$data[p15$data$UMAP_1 < -2.5 & p15$data$UMAP_1 > -7 & p15$data$UMAP_2 > 5 & p15$data$UMAP_2 < 8 & p15$data$subtype == 'Glutamatergic_L5_PT',]


idents = rep('0', dim(visualCortex.10xintegrated)[2])
names(idents) = colnames(visualCortex.10xintegrated)
idents[visualCortex.10xintegrated$subtype == 'Glutamatergic_L5_PT'] = '1'
idents[rownames(oddCluster)] = '2'
Idents(visualCortex.10xintegrated) = idents

cluster_markers <- FindMarkers(visualCortex.10xintegrated, ident.1 = '2', ident.2 = '1')
...
all_genes <- rep(1, dim(visualCortex.10xintegrated)[2])
names(all_genes) = rownames(visualCortex.10xintegrated)
all_genes[rownames(cluster_markers)] = 0
...
Let's save the Seurat object used in this analysis, so we can easily load it in the future (although we need to ignore this one from commits to github because its too large):

```{r, include = TRUE, message = FALSE, warning = FALSE, cache = FALSE}

saveRDS(visualCortex.list, file = 'visualCortex_snSeq_SeuratObject.rds')
